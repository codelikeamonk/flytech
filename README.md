...
## A debugging note: writes after the first one, on an iOS simulator

While testing against nRF Connect for iOS as the simulated peripheral, the first write to the Control characteristic after each fresh connection succeeded, but every write after that on the same connection failed with `InvalidStateError`, even seconds apart with no overlapping calls — until disconnecting and reconnecting, which reset the pattern (next write succeeds, then fails again). The UUIDs, encoding, and write call were identical between the successful and failing attempts, so this wasn't a bug in the client logic; it points at nRF Connect for iOS's peripheral/server implementation, which Nordic's own developers have publicly described as the less mature half of the app (Android's GATT server feature is the one documented as reliable). `device.gatt.connected` kept reporting `true` through this whole failure window, which is itself the interesting part: the browser's cached connection state and the actual link health had drifted apart, and nothing here would have caught that without watching it happen.

Given that, the app now treats an `InvalidStateError` or `NetworkError` on a read/write as a possible sign of a silently stale connection, not just a one-off failure: it forces a clean `gatt.disconnect()`, which hands off to the same auto-reconnect path used for a real drop. In practice this turns "every other write quietly fails forever" into "an occasional automatic reconnect, then writes keep working." If you have a spare Android phone, switching the simulator to nRF Connect for Android should avoid the underlying issue entirely rather than just recovering from it — worth trying if you want a completely clean test run for the screen recording.
